import { getMaskedApiKeyOverview } from '$lib/server/ai/model-selector';
...
export const GET: RequestHandler = async ({ request, locals }) => {
	...
	const denied = forbiddenWithoutPermission(
		session.user,
		'MANAGE_AI_KEYS',
		'Keine Berechtigung für die Einsicht in API-Schlüssel.',
		'PF_MANAGE_AI_KEYS_FORBIDDEN'
	);
	if (denied) return denied;

	return json(getMaskedApiKeyOverview());
};
